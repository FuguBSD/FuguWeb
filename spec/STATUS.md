...
| [WEB-ACTIONS](web.md#web-actions) | partial | —       | [keys-rotate.yml](../.github/workflows/keys-rotate.yml) runs one step of one purpose, and it composes [keys-slot](../actions/keys-slot/action.yml) and [keys-store](../actions/keys-store/action.yml). [keys-rotate.t](../t/ci/keys-rotate.t) holds the order and each guard. WEB-ACTIONS-11 pins each action of another owner to a commit, and the two actions of this repository carry `@main`: they land in the commit that the workflow lands in, and a commit cannot name itself. |
