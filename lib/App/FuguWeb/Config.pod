...
The binding files of the key directory, in name order. A binding is
the signature of one key file by another key of the directory. Each
entry is a hashref:
...
whose signer are both keys of the description, and
