...
remove the staging directory, every file the site no longer holds, and
...
A key directory that the description no longer names leaves its own
directory behind. C<rmdir> refuses a directory that still holds a
name, so the walk needs no second test. A directory that the site
still uses stays, and the output directory itself always stays.
