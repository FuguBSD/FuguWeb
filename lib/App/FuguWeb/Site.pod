...
file below a prefix of the key directory. It keeps every other entry
and reports it, so an operator directory in the output survives a
build.

The build removes an empty directory of a key directory prefix, and it
keeps every other one and reports it. C<rmdir> refuses a directory that
still holds a name, so the walk needs no second test, and the output
directory itself always stays.
...
The rule is the inventory. A plain file of the top level is one that a
build writes, whatever the description names today. A renamed manual
leaves its old page there. A file below the root belongs to the key
directory, so the site must name it. A directory must hold a name of
the site, and a symlink is never a thing that a build wrote. The clean
refuses every other entry, and it names it.
