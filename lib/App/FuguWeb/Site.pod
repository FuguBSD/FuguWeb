...
operator directory in the output survives a build.

The build removes every empty directory of the output, and the output
directory itself always stays. An empty directory holds no content, so
no removal there can lose one. C<rmdir> refuses a directory that still
holds a name, so the walk needs no second test.
...
The site names nothing there any more, so C<clean> refuses it and the
checks report it. Remove the directory, or name the block again.
